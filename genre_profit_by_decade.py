...
def process_folder(folder_name):
    ...
    for filename in sorted(os.listdir(folder_name)):
        ...
        required_columns = ["genres", "budget", "revenue"]
        ...
        # Keep only rows with valid financial data
        df = df[df["budget"].notna() & df["revenue"].notna()].copy()

        if df.empty:
            print(f"Skipping {filename}: no valid budget/revenue values")
            continue
        ...
        # Top 5 genres ranked by TOTAL profit
        grouped = grouped.sort_values(
            by=["total_profit", "average_profit_per_movie", "movie_count"],
    ...
    if results:
        ...
        output_path = os.path.join(folder_name, "top5_genres_by_profit.csv")
